...
| Widget delivery             | The target Worker build contains and serves `public/board.js` from the same deployment as the API. The local browser fixture's `/board.js` is dummy-host served, so it is not deployed-asset proof. | [2026-08-15 Staging Dogfood](staging-dogfood-results/2026-08-15.md), then `npm run deploy:smoke -- --url <worker-url> --expect-environment production` against the target Worker | Staging baseline proven; operator-pending for each beta install.              |
| Worker deploy proof         | The deployed Worker answers `/health`, serves `/board.js`, and reports the expected environment.                                                                                                    | [2026-08-15 Staging Dogfood](staging-dogfood-results/2026-08-15.md), then `npm run deploy:smoke -- --url <worker-url> --expect-environment production`                           | Staging baseline proven; operator-pending for each beta install.              |
...
The 2026-08-15 GitHub API audit reported zero open CodeQL, Dependabot, or secret-scanning alerts.
That result is a dated repository-security signal, not a substitute for rerunning the checks or the
target-install runtime proof.
