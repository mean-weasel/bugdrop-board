...
| Widget delivery             | The target Worker build contains and serves `public/board.js` from the same deployment as the API.                                                                      | `npm run build:widget`, `npm run deploy:check:production`, `npm run deploy:smoke -- --url <worker-url> --expect-environment production`                         | Go when the target Worker build and deployed asset smoke pass.                |
